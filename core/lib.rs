...
impl Connection {
    ...
    pub fn track_program(&self, program_rc: &Rc<Program>) {
        ...
        let mut new_node_ptr = NonNull::new(program_mut_ptr).unwrap();

        let current_head = self.program_head.get();

        unsafe {
            new_node_ptr.as_mut().next_program.set(current_head);
            new_node_ptr.as_mut().prev_program.set(None);

            if let Some(mut old_head) = current_head {
                old_head.as_mut().prev_program.set(Some(new_node_ptr));
            }
    ...
    fn untrack_program(&self, program: &mut Program) {
        let self_ptr = NonNull::from(&mut *program);
        let prev_of_removed_opt = program.prev_program.get();
        let next_of_removed_opt = program.next_program.get();

        unsafe {
            if let Some(prev_node_ptr) = prev_of_removed_opt {
                prev_node_ptr.as_ref().next_program.set(next_of_removed_opt);
            } else if self.program_head.get() == Some(self_ptr) {
                self.program_head.set(next_of_removed_opt);
            }

            if let Some(next_node_ptr) = next_of_removed_opt {
                next_node_ptr.as_ref().prev_program.set(prev_of_removed_opt);
            }
        }
